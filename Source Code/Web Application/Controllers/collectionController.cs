    ...
    public class collectionController : Controller
    {
        ...
        public ActionResult GetSheets(string file)
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    ViewBag.Validation = "If file has password, remove it then try to upload it again.";
                }
                else
                {
        ...
        public ActionResult CheckToImport(byte SchemaID, string file, string SheetName)
        {
            ...
            if (SheetName.Length > 0)
            {
                ...
                if (NumberOfColOfExcel >= NumberOfColOfSchema)
                {
                    ...
                    int? CollectorOfficeIndexNullable = Convert.ToInt16(_enty.Sch_SchemaColList.Where(q => q.SchemaID == SchemaID && q.ColumnName == "Collector Office").Select(q => q.ColumnIndex).FirstOrDefault());
                    int? TeamleaderIndexNullable = Convert.ToInt16(_enty.Sch_SchemaColList.Where(q => q.SchemaID == SchemaID && q.ColumnName == "Team Leader").Select(q => q.ColumnIndex).FirstOrDefault());
                    int? TeamleaderCodeIndexNullable = Convert.ToInt16(_enty.Sch_SchemaColList.Where(q => q.SchemaID == SchemaID && q.ColumnName == "Team Leader Code").Select(q => q.ColumnIndex).FirstOrDefault());
                    int? deputyIndexNullable = Convert.ToInt16(_enty.Sch_SchemaColList.Where(q => q.SchemaID == SchemaID && q.ColumnName == "deputy Team Leader Name").Select(q => q.ColumnIndex).FirstOrDefault());
                    int? deputyCodeIndexNullable = Convert.ToInt16(_enty.Sch_SchemaColList.Where(q => q.SchemaID == SchemaID && q.ColumnName == "deputy Team Leader Code").Select(q => q.ColumnIndex).FirstOrDefault());
                    ...
                    int? CityIndexNullable = Convert.ToInt16(_enty.Sch_SchemaColList.Where(q => q.SchemaID == SchemaID && q.ColumnName == "City").Select(q => q.ColumnIndex).FirstOrDefault());
                    ...
                    if (CollectorOfficeIndexNullable != null && CollectorOfficeIndexNullable != 0)
                        Params.Add(dtable.Columns[(int)CollectorOfficeIndexNullable - 1].ColumnName);
                    if (TeamleaderIndexNullable != null && TeamleaderIndexNullable != 0)
                        Params.Add(dtable.Columns[(int)TeamleaderIndexNullable - 1].ColumnName);
                    if (TeamleaderCodeIndexNullable != null && TeamleaderCodeIndexNullable != 0)
                        Params.Add(dtable.Columns[(int)TeamleaderCodeIndexNullable - 1].ColumnName);
                    if (deputyIndexNullable != null && deputyIndexNullable != 0)
                        Params.Add(dtable.Columns[(int)deputyIndexNullable - 1].ColumnName);
                    if (deputyCodeIndexNullable != null && deputyCodeIndexNullable != 0)
                        Params.Add(dtable.Columns[(int)deputyCodeIndexNullable - 1].ColumnName);
                    if (ProductIndexNullable != null && ProductIndexNullable != 0)
                        Params.Add(dtable.Columns[(int)ProductIndexNullable - 1].ColumnName);
                    if (CityIndexNullable != null && CityIndexNullable != 0)
                        Params.Add(dtable.Columns[(int)CityIndexNullable - 1].ColumnName);
                    ...
                    try
                    {
                        if (ProductIndexNullable != null && ProductIndexNullable != 0)
                        {
                            ...
                            //System.Data.DataView view2 = new System.Data.DataView(dtable);
                            //System.Data.DataTable selected = view.ToTable("Selected", false, ProductColName);
                            List<string> ProductValues = selected.AsEnumerable().Select(s => s.Field<string>(ProductColName)).Distinct().ToList();
                            var productnames = _enty.Products.Select(s => s.ProductName.Trim()).ToList();
                            foreach (var productitem in ProductValues)
                            {
                                if (productitem != null)
                                {
                                    if (!productnames.Contains(productitem.Trim()))
                                    {
                                        ViewBag.Product = "Invalid data in product Column, Data invalid is product Name: " + productitem;
                                        Error = true;
                                        break;
                                    }
                                }
                    ...
                    {
                        ViewBag.Product = ex.Message.ToString();
                        Error = true;
                    }
                    #endregion
                    #region city
                    try
                    {
                        if (CityIndexNullable != null && CityIndexNullable != 0)
                        {
                            int CityIndex = Convert.ToInt16(CityIndexNullable);
                            string CityColName = dtable.Columns[CityIndex - 1].ColumnName;
                            List<string> CityValues = selected.AsEnumerable().Select(s => s.Field<string>(CityColName)).Distinct().ToList();
                            var Citynames = _enty.Cities.Select(s => s.CityName.Trim()).ToList();
                            foreach (var Cityitem in CityValues)
                            {
                                if (Cityitem != null)
                                {
                                    if (!Citynames.Contains(Cityitem.Trim()))
                                    {
                                        ViewBag.Collectors = "Invalid data in City Column, Data invalid is City Name: " + Cityitem;
                                        Error = true;
                                        break;
                                    }
                                }
                    ...
                    try
                    {
                        if (TeamleaderIndexNullable != null && TeamleaderCodeIndexNullable != null && TeamleaderIndexNullable != 0 && TeamleaderCodeIndexNullable != 0)
                        {
                            var MemberList = selected.AsEnumerable().Select(s => new MemberList { Name = s.Field<string>(dtable.Columns[(int)TeamleaderIndexNullable - 1].ColumnName), Code = s.Field<object>(dtable.Columns[(int)TeamleaderCodeIndexNullable - 1].ColumnName) }).ToList();
                            var UinqueMemberList = MemberList.Select(m => new { m.Name, m.Code }).Distinct().ToList();
                           
                            var DBCollectors = _enty.Employees.Select(s => s).ToList();


                            foreach (var Teamleader in UinqueMemberList)
                            {
                                if (Teamleader.Code != null && Teamleader.Name != null)
                                {
                                    var TeamleaderExist = DBCollectors.Where(a => a.Code == Teamleader.Code.ToString() && a.Name == Teamleader.Name).FirstOrDefault();

                                    if (TeamleaderExist == null)
                                    {
                                        ViewBag.Collectors = "Invalid data in Team leader Name Column or in Team leader Code Column, Data invalid is Team leader Name: " + Teamleader.Name + " ,Team leader Code: " + Teamleader.Code;
                                        Error = true;
                                        break;
                                    }
                                }
                    ...
                    try
                    {
                        ...
                        if (CollectorIndexNullable != null && CollectorCodeIndexNullable != null && CollectorIndexNullable != 0 && CollectorCodeIndexNullable != 0)
                        {
                            ...
                            if (CollectorOfficeIndexNullable != null && CollectorOfficeIndexNullable != 0)
                            {
                                MemberList = selected.AsEnumerable().Select(s => new MemberList { Name = s.Field<string>(dtable.Columns[(int)CollectorIndexNullable - 1].ColumnName), Code = s.Field<object>(dtable.Columns[(int)CollectorCodeIndexNullable - 1].ColumnName), Office = s.Field<string>(dtable.Columns[(int)CollectorOfficeIndexNullable - 1].ColumnName) }).ToList();
                            }
                            else
                            {
                                MemberList = selected.AsEnumerable().Select(s => new MemberList { Name = s.Field<string>(dtable.Columns[(int)CollectorIndexNullable - 1].ColumnName), Code = s.Field<object>(dtable.Columns[(int)CollectorCodeIndexNullable - 1].ColumnName) }).ToList();
                            }
                            var UinqueMemberList = MemberList.Select(m => new { m.Name, m.Code, m.Office }).Distinct().ToList();
                            ...
                            ////}
                            ////var Unique_Names = new HashSet<string>(CollectorNames).Where(a => !string.IsNullOrEmpty(a)).Distinct().ToList();
                            ////var Unique_Codes = new HashSet<string>(CollectorCodes).Where(a => !string.IsNullOrEmpty(a)).ToList();
                            var DBCollectors = _enty.Employees.Select(s => s).ToList();
                            ////var DBCollectorNames = DBCollectors.Select(s => s.Name).ToList();
                            ////var DBCollectorCodes = DBCollectors.Select(s => s.Code).ToList();
                            var CollectorExist = new Employee();
                            foreach (var Collector in UinqueMemberList)
                            {
                                if (Collector.Code != null && Collector.Name != null)
                                {
                                    if (CollectorOfficeIndexNullable != null && CollectorOfficeIndexNullable != 0)
                                    {
                                        CollectorExist = DBCollectors.Where(a => a.Code == Collector.Code.ToString() && a.Name == Collector.Name && a.Pre_Offices.OfficeTitle == Collector.Office).FirstOrDefault();
                                    }
                                    else
                                    {
                                        CollectorExist = DBCollectors.Where(a => a.Code == Collector.Code.ToString() && a.Name == Collector.Name).FirstOrDefault();
                                    }
                                    if (CollectorExist == null)
                                    {
                                        ViewBag.Collectors = "Invalid data in Collector Name Column or in Collector Code Column, Data invalid is Collector Name: " + Collector.Name + " ,Collector Code: " + Collector.Code + (Collector.Office != null ? " ,Collector Office: " + Collector.Office : "");
                                        Error = true;
                                        break;
                                    }
                                }
                    ...
                    try
                    {
                        if (deputyIndexNullable != null && deputyCodeIndexNullable != null && deputyIndexNullable != 0 && deputyCodeIndexNullable != 0)
                        {
                            var MemberList = selected.AsEnumerable().Select(s => new MemberList { Name = s.Field<string>(dtable.Columns[(int)deputyIndexNullable - 1].ColumnName), Code = s.Field<object>(dtable.Columns[(int)deputyCodeIndexNullable - 1].ColumnName) }).ToList();
                            var UinqueMemberList = MemberList.Select(m => new { m.Name, m.Code }).Distinct().ToList();

                            var DBCollectors = _enty.Employees.Select(s => s).ToList();


                            foreach (var deputy in UinqueMemberList)
                            {
                                if (deputy.Code != null && deputy.Name != null)
                                {
                                    var deputyExist = DBCollectors.Where(a => a.Code == deputy.Code.ToString() && a.Name == deputy.Name).FirstOrDefault();

                                    if (deputyExist == null)
                                    {
                                        ViewBag.Collectors = "Invalid data in deputy Name Column or in deputy Code Column, Data invalid is deputy Name: " + deputy.Name + " ,deputy Code: " + deputy.Code;
                                        Error = true;
                                        break;
                                    }
                                }
        ...
        public ActionResult SaveAppendPayments(FormCollection frm)
        {
            try
            {
                ...
                string SheetName = frm["SheetName"];
                string PKFieldCol = frm["PKColumn"].Trim(); 
                string PKAliasCol = frm["PK"].Trim(); 
                string PaymentColumnCol = frm["PaymentColumn"].Trim();
                ...
                if (SheetName.Length > 0 && PKFieldCol.Length > 0 && PKAliasCol.Length > 0 && PaymentColumnCol.Length > 0)
                {
                    ...
                    {
                        dtable.Columns[i - 1].ColumnName = dtable.Columns[i - 1].ColumnName.Trim();
                    }
                    ...
                    foreach (DataRow R in dtable.Rows)
                    {
                        ...
                        if (!string.IsNullOrEmpty(R[PaymentColumnCol].ToString()))
                        {
                                ...
                                string PaymentDate = "";
                                if (dtable.Columns.Contains("Payment Date"))
                                {
                                  PaymentDate = string.IsNullOrEmpty(R["Payment Date"].ToString()) == false ? Convert.ToDateTime(R["Payment Date"]).ToShortDateString() : "";
                                }
            ...
            if (NumberOfColOfExcel >= NumberOfColOfSchema)
            {
                try
                {
                    ...
                    for (int j = 0; j < dtable.Rows.Count; j++)
                    {
                        ...
                        for (int i = 1; i <= dtable.Columns.Count; i++)
                        {
                            if (i > 200)  //buz last column is Col200
                            {
                                break;
                            }
                            sql = sql + ",Col" + i;
                        }
                        //var xx = sql.Split(',').ToList();
                        //values
                        sql = sql + ") values(" + SchemaID + " , " + DomainID + " , " + VersionNumber + "";

                        for (int i = 0; i < dtable.Columns.Count; i++)
                        {
                            if (i > 199)  //buz last column is Col200
                            {
                                break;
                            }
...
+ " and (dx.Col" + DomainTypeIndex + " ='active' or dx.col" + DomainTypeIndex + " ='w/o') and  " +
                        ...
                        {
                            MonthYear =string.IsNullOrEmpty(s.Field<string>("monthyear")) == false ?
                               (months == null ? s.Field<string>("monthyear").Split('/')[2] : s.Field<string>("monthyear").Split('/')[0]) : (months == null ? s.Field<string>("DlMonthYear").Split('-')[1] : s.Field<string>("DlMonthYear").Split('-')[0]),
                ...
                if (result.Count() > 0)
                {
                    ...
                    if (type[0] == 1 || type[0] == 3)
                    {
                        ViewBag.TeamLeaderReport = result.Where(c => int.Parse(c.MonthYear) <= years.Max()).GroupBy(c => new { c.GroupId, c.TeamLeaderName, c.MonthYear, c.DomainType, c.Product, c.AgencyTitle }).Select(c => new DomainReoprtbyTeamLeader()
                        {
                            ...
                            Payment = c.Sum(s => s.Payment!=""? double.Parse(s.Payment) : 0).ToString(),
                            Balance = c.Sum(s =>s.Balance !=""? double.Parse(s.Balance):0).ToString(),
                            PastDue = c.Sum(s =>s.PastDue !=""? double.Parse(s.PastDue):0).ToString()
            ...
            if (NumberOfColOfExcel >= NumberOfColOfSchema)
            {
                try
                {
                    ...
                    for (int j = 0; j < dtable.Rows.Count; j++)
                    {
                        ...
                        for (int i = 1; i <= dtable.Columns.Count; i++)
                        {
                            if (i > 250)  //buz last column is Col250
                            {
                                break;
                            }
                            sql = sql + ",Col" + i;
                        ...
                        for (int i = 0; i < dtable.Columns.Count; i++)
                        {
                            if (i > 249)  //buz last column is Col250
                            {
                                break;
                            }
                            sql = sql + ",N'" + dtable.Rows[j][i].ToString().Trim().Replace("'", "''") + "'";
        ...
        public ActionResult GetPaymentReport(List<int> years = null, List<int> months = null, List<int> type = null, List<string> bankNames = null)
        {
            ...
            foreach (var schemaID in schemasID)
            {
                ...
                if (NameIndex != null && accountNameIndex != null && DomainTypeIndex != null && PastdueIndex != null && BalanceIndex != null)
                {
                    ...
                    string Querystring = " select dl.MonthYear as DlMonthYear, dl.AgencyTitle AgencyTitle ,  dl.BranchTitle  BranchTitle ,ap.PaymentDate monthyear , dx.Col" + NameIndex +
                        ...
                        + " ,count (CASE WHEN dx.col" + DomainTypeIndex + " = 'W/O' THEN  1 end) as WOAcountsNum" +
                   ...
                   " ,SUM (CASE WHEN dx.col" + DomainTypeIndex + " = 'Active' THEN ap.PaymentValue  ELSE 0 end) as ActivePayment"
                        ...
                        + " ,count (CASE WHEN dx.col" + DomainTypeIndex + " = 'Active' THEN  1  end) as ActiveAccountsNum" +
            ...
            if (result.Count() > 0)
            {
                ...
                ViewBag.BankReport = result.Where(c => int.Parse(c.MonthYear) <= years.Max()).GroupBy(c => new { c.AgencyTitle, c.MonthYear, c.TeamLeaderName, c.BranchName }).Select(c => new BankReoprtbyTeamLeader()
                {
                    ...
                    ActiveOs = c.Sum(s => s.ActiveOs != "" ? double.Parse(s.ActiveOs) : 0).ToString(),
                    ActivePD = c.Sum(s => s.ActivePD!=""? double.Parse(s.ActivePD):0).ToString(),
                    WOAcountsNum = c.Sum(s => s.WOAcountsNum),
                    WOPayment = c.Sum(s => decimal.Parse(s.WOPayment)).ToString(),
                    WOOs = c.Sum(s => s.WOOs != "" ? double.Parse(s.WOOs) : 0).ToString(),
                    WOPD = c.Sum(s => s.WOPD != "" ? double.Parse(s.WOPD) : 0).ToString(),
                }).ToList();
            ...
            foreach (var schemaID in schemasID)
            {
                ...
                if (teamLeaderNameIndex != null && collectorNameIndex != null && accountNameIndex != null && DomainTypeIndex != null && PastdueIndex != null && BalanceIndex != null)
                {
                   ...
                   " ,SUM (CASE WHEN dx.col" + DomainTypeIndex + " = 'Active' THEN ap.PaymentValue  ELSE 0 end) as ActivePayment"
                         + " ,SUM (CASE WHEN dx.col" + DomainTypeIndex + " = 'Active' THEN Try_Cast(dx.Col" + PastdueIndex + " AS float)  ELSE 0 end) as ActivePD"
                        + " ,SUM (CASE WHEN dx.col" + DomainTypeIndex + " = 'Active' THEN Try_Cast(dx.Col" + BalanceIndex + " AS float)  ELSE 0 end) as ActiveBalance"
                        + " ,count (CASE WHEN dx.col" + DomainTypeIndex + " = 'Active' THEN  1 end) as ActiveAcountsNum" +
            ...
            if (result.Count() > 0)
            {
                ...
                if (type[0] == 1)
                {
                    ViewBag.BankReport = result.Where(c => int.Parse(c.MonthYear) <= years.Max()).GroupBy(c => new { c.CollectorName, c.TeamLeaderName, c.AgencyTitle, c.MonthYear }).Select(c => new PaymentPerBank()
                    {
                        ...
                        AgencyTitle = c.Key.AgencyTitle,
                        MonthYear = c.Key.MonthYear,
                        ActivePayment = c.Sum(s => s.ActivePayment != "" ?double.Parse(s.ActivePayment):0).ToString(),
                        ActiveOs = c.Sum(s => s.ActiveOs != "" ? double.Parse(s.ActiveOs) : 0).ToString(),
                        ActivePD = c.Sum(s => s.ActivePD != "" ? double.Parse(s.ActivePD) : 0).ToString(),
                        ActiveNumberOfAccounts = c.Sum(s => s.ActiveNumberOfAccounts),
                        WOPayment = c.Sum(s => s.WOPayment != "" ? double.Parse(s.WOPayment) : 0).ToString(),
                        WOOs = c.Sum(s => s.WOOs != "" ? double.Parse(s.WOOs) : 0).ToString(),
                        WOPD = c.Sum(s => s.WOPD!=""? double.Parse(s.WOPD):0).ToString(),
                ...
                {
                    ViewBag.BankReport = result.Where(c => int.Parse(c.MonthYear) <= years.Max()).GroupBy(c => new { c.CollectorName, c.AgencyTitle, c.MonthYear }).Select(c => new PaymentPerBank()
                    {
