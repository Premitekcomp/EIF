    ...
    public class adminController : Controller
    {
        ...
        public ActionResult NewUser(FormCollection frm)
        {
            ...
            try
            {
                ...
                if (ViewBag.Msg == null)
                {
                    int res = _enty.pre_UsersInsert(DisplayName, UserName, Password, Image, Convert.ToInt16(GroupID), true, DateTime.Now.Date, Convert.ToInt16(Session["ID"]) , BranchID);
                    if (res > 0)
                    {
        ...
        public ActionResult EditUser(FormCollection frm)
        {
            ...
            try
            {
                ...
                if (ViewBag.Msg == null)
                {
                    int res = _enty.pre_UsersEdit(id, DisplayName, UserName, Convert.ToInt16(GroupID), DateTime.Now.Date, Convert.ToInt16(Session["ID"]) ,BranchID);
                    if (res > 0)
                    {
                        ViewBag.saved = "Data saved successfully";
                    }
...
if (active == "0")
        ...
        public ActionResult Members()
        {
            ...
            if (Convert.ToInt16(Session["ID"]) > 0 || Session["ID"] == null)
            {
                ...
                {                  
                    //Team Data
                    var Data = _enty.Config_MembersList.Select(q => q).ToList();
                    if (Data != null)
                    {
                        ViewBag.Data = Data;
                    }
        ...
        public ActionResult NewMember(string Name , string Code)
        {
            if ( Name.Length > 0)
            {
            ...
            //save new agency
            return PartialView("_AdminMembers");
        }
        ...
        public ActionResult DeleteMember(int id)
        {
            ...
            if (member != null)
            {
                _enty.Entry(member).State = System.Data.Entity.EntityState.Deleted;
                _enty.SaveChanges();
