...
namespace EIF.Controllers
{
    public class collectionController : Controller
    {
        ...
        public ActionResult AppendPayments(int id)
        {
            if (Session["ID"] == null)
            {
                Session["ID"] = 0;
                ...
                if (SheetName.Length > 0 && PKFieldCol.Length > 0 && PKAliasCol.Length > 0 && PaymentColumnCol.Length > 0)
                {
                    ...
                    foreach (DataRow R in dtable.Rows)
                    {
                        ...
                        if (!string.IsNullOrEmpty(R[PaymentColumnCol].ToString()))
                        {
                            ...
                            var result = _enty.Dom_Dom_AccountsPaymentsInsert(DomainId, PKField, PKAliasCol, Convert.ToInt16(VersionNumber), PaymentColumn, PaymentDate);
                        }
                    }
                    ViewBag.saved = "Data saved successfully";
        ...
        public ActionResult GetTeamLeadersReport(List<int> months, List<int> years, List<string> teamLeaderNames, List<int> officesId = null, List<int> type = null, List<int> caseDivision = null)
        {
            ...
            {
                if (caseDivision[0] == 1)
                    caseDivisionFilter = " and Pre_groupsMembers.GroupID !=" + 0;
                if (caseDivision[0] == 2)
                    caseDivisionFilter = " and Pre_groupsMembers.GroupID =" + 6;
            ...
            if (officesId != null)
            {
                officeFilter = "and(Pre_OfficesMembers.OfficeID = " + officesId[0];
                for (var index = 1; index < officesId.Count; index++)
                {
                    officeFilter += "or Pre_OfficesMembers.OfficeID= " + officesId[index];
                }
            ...
            else
            {
                ...
                foreach (var schemaID in schemasID)
                {
                    ...
                    if (NameIndex != null && CustNameIndex != null && ProductIndex != null && DomainTypeIndex != null && PastdueIndex != null && BalanceIndex != null)
                    {
                        ...
                        {
                            caseDivisionJoin = "  left join Pre_Users on Pre_Users.DisplayName=dx.col" + NameIndex +
                                                  "  left  join Pre_groupsMembers on Pre_Users.UserID=Pre_groupsMembers.UserID ";
                        }

                        string connstring = System.Configuration.ConfigurationManager.ConnectionStrings["EIF"].ConnectionString;
                        string Querystring = "select distinct dl.MonthYear as DlMonthYear, Pre_groupsMembers.GroupID GroupID , dl.AgencyTitle AgencyTitle,ap.PaymentDate monthyear , dx.Col" + NameIndex +
                            " TeamLeaderName, dx.Col" + ProductIndex + " Product , dx.Col" + DomainTypeIndex +
                            " DomainType ,SUM (ap.PaymentValue) Payment , COUNT(ap.PaymentValue) PaymentCount ,COUNT(dx.Col" + CustNameIndex + ") CountofCustomers " +
                         " ,SUM(TRY_CAST(dx.Col" + BalanceIndex + " AS float)) Balance " +
                          " ,SUM(TRY_CAST(dx.Col" + PastdueIndex + " AS float)) PastDue " +
                        "from Dom_DomainList dl  join Dom_DomainsDataExtra dx on dl.DomainID = dx.DomainID  left join Dom_AccountsPayments ap on dx.RecordSerial = ap.RecordSerial" + caseDivisionJoin + officeJoin + " where (dx.Col" + DomainTypeIndex + " ='active' or dx.col" + DomainTypeIndex + " ='w/o') and  " +
                            "dx.Col" + NameIndex + " != ''"
                            + " and dx.SchemaID=" + schemaID
                            + filter + caseDivisionFilter + officeFilter + teamLeaderFilter
                            + " group by dl.MonthYear, Pre_groupsMembers.GroupID, dl.AgencyTitle,ap.PaymentDate, dl.DomainID , dx.Col" + NameIndex + " , dx.Col" + DomainTypeIndex + " , dx.Col" + ProductIndex;                      
        ...
        public ActionResult GetCollectorCodes(string name)
        {
            ...
            var names = _enty.Reports_GetNames(name, "Collector Code");

            return Json(names.Where(c=>c!=null).Select(c => new { id = c.ToString(), text = c.ToString() }).Distinct().ToList(), JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult GetReportCollector(List<int> years = null, List<int> months = null, List<string> collectorNames = null, List<string> collectorCodes = null, List<int> officesId = null, List<int> type = null, List<int> caseDivision = null)
        {
            ...
            List<CollectorName> names = new List<CollectorName>();
            var officeJoin = "";
            var officeFilter = "";
            var caseDivisionJoin = "";
            var caseDivisionFilter = "";
            ...
            else
            {
                ...
                foreach (var schemaID in schemaIdsList)
                {
                    ...
                    if (officesId != null)
                    {
                        officeJoin = "  join Pre_Users on Pre_Users.DisplayName=dx.col" + NameIndex +
                                               "  join Pre_OfficesMembers on Pre_Users.UserID=Pre_OfficesMembers.UserID ";
                    }

                    if (caseDivision == null ? false : caseDivision.Count > 0)
                    {
                        caseDivisionJoin = "  join Pre_Users on Pre_Users.DisplayName=dx.col" + NameIndex +
                                              "  join Pre_groupsMembers on Pre_Users.UserID=Pre_groupsMembers.UserID ";
                    }
                    ...
                    if (CodeIndex != null && NameIndex != null && CustNameIndex != null && ProductIndex != null && DomainTypeIndex != null && PastdueIndex != null && BalanceIndex != null)
                    {
                        ...
                        string Querystring = 
                          ...
                          "select DISTINCT dl.DomainTitle, dl.MonthYear as DlMonthYear,  dl.AgencyTitle, ap.PaymentDate monthyear, dx.Col" + CodeIndex + " CollectorCode , dx.Col" + NameIndex +
                            " CollectorName, dx.Col" + ProductIndex + " Product , dx.Col" + DomainTypeIndex + " DomainType ,SUM (ap.PaymentValue) Payment , COUNT(ap.PaymentValue) PaymentCount ,COUNT(dx.Col" + CustNameIndex + ") CountofCustomers" +

                         " ,SUM(Try_Cast(dx.Col" + BalanceIndex + " AS float)) Balance " +
                          " ,SUM(Try_Cast(dx.Col" + PastdueIndex + " AS float)) PastDue "
                        + " from Dom_DomainList dl   join  Dom_DomainsDataExtra  dx on dl.DomainID = dx.DomainID left join Dom_AccountsPayments ap on dx.RecordSerial = ap.RecordSerial " +
                        caseDivisionJoin + officeJoin +
                        " where dx.SchemaID= " + schemaID
                        + " and  (dx.Col" + DomainTypeIndex + " = 'active' or  dx.Col" + DomainTypeIndex + " = 'w/o' ) " 
                        + "and  dx.Col" + CodeIndex + "!=''" + " and dx.Col" + NameIndex + "!=''" +
                         filter + collectorNameFilter + collectorCodeFilter + officeFilter + caseDivisionFilter+
                            " group by dl.MonthYear ,dl.DomainTitle, dl.AgencyTitle, ap.PaymentDate, dl.DomainID, dx.Col" + CodeIndex + " , dx.Col" + NameIndex + " , dx.Col" + DomainTypeIndex + " , dx.Col" + ProductIndex;
                        SqlDataAdapter adapter = new SqlDataAdapter(Querystring, connstring);
                        DataSet ds = new DataSet();
                        var num = 0.0;
                        adapter.Fill(ds, "PaymentTable");
        ...
        public ActionResult GetPaymentReport(List<int> years = null, List<int> months = null, List<string> bankNames = null)
        {
        ...
        public ActionResult GetDailyReportPerBranch(List<int> months = null, List<int> days = null, List<int> years = null, List<string> bankNames = null)
        {
            ...
            if ((months != null ? months.Count == 0 : true) && (years != null ? years.Count > 0 : false))
            {
                filter = "and(ap.PaymentDate like " + "'%" + years[0] + "'";

                for (var index = 1; index < years.Count; index++)
                {
                    filter += "or ap.PaymentDate like " + "'%" + years[index] + "'";
                }
            ...
            foreach (var schemaID in schemasID)
            {
                ...
                if (NameIndex != null && accountNameIndex != null && DomainTypeIndex != null && PastdueIndex != null && BalanceIndex != null)
                {
                    ...
                    string connstring = System.Configuration.ConfigurationManager.ConnectionStrings["EIF"].ConnectionString;
                    string Querystring = " select dl.AgencyTitle AgencyTitle ,  dl.BranchTitle  BranchTitle ,ap.PaymentDate monthyear , dx.Col" + NameIndex +
                        " TeamLeaderName, dx.Col" + DomainTypeIndex + " DomainType ," +
                        "SUM (CASE WHEN dx.col" + DomainTypeIndex + " = 'W/O' THEN ap.PaymentValue  ELSE 0 end) as WOPayment , "
                   ...
                   " from Dom_DomainList dl left join Dom_DomainsDataExtra dx on dl.DomainID = dx.DomainID  left  join Dom_AccountsPayments ap on dx.RecordSerial = ap.RecordSerial" + " where dx.Col" + NameIndex + " != ''"
                        + " and dx.SchemaID=" + schemaID 
                        + filter + bankNameFilter + " and  (dx.Col" + DomainTypeIndex + "='" + "W/O'" + " or dx.Col" + DomainTypeIndex + "='" + "Active')"
                        + " group by dl.BranchTitle , dl.AgencyTitle,ap.PaymentDate, dl.DomainID , dx.Col" + NameIndex + " , dx.Col" + DomainTypeIndex;
