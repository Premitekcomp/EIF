...
namespace EIF.Helper
{
    public class Get
    {
       ...
       public SelectList OfficesList(int? id, int? Choosen)
       {
          var list = new List<Pre_Offices1>();
          if (id == 0)
          {
              list = _enty.Pre_Offices1.Where(q => q.IsDeleted == false && q.IsActive == true).Select(q => q).ToList();
          }
          else
          {
              list = _enty.Pre_Offices1.Where(q => q.IsDeleted == false && q.IsActive == true).Select(q => q).ToList();
          ...
          if (Choosen != null)
          {
              return new SelectList((list.Select(q => q)).ToList(), "OfficeId", "OfficeTitle", Choosen);
          }
          else
          {
              return new SelectList((list.Select(q => q)).ToList(), "OfficeId", "OfficeTitle");
          }
